1️⃣ Extension description (store listing)

Detects Google Meet, Microsoft Teams, and Zoom tabs and triggers user-configured local or remote endpoints (HTTP or WebSocket) to control external indicators such as “On Air” lights or smart home automations.

2️⃣ Permission justification (review form)

tabs: used to detect meeting tabs

storage: used to save user configuration

optional_host_permissions:
“Used to send HTTP requests to user-specified endpoints (e.g., Home Assistant, smart devices) only after explicit user configuration.”

3️⃣ Privacy disclosure (required)

This extension does not collect, store, or transmit personal data.
All network requests are initiated by the user and target endpoints configured by the user.
